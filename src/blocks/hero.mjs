...
const renderSlide=(slide,index,total)=>`<figure class="hero-slide hero-slide-${esc(slide.id)}" data-hero-slide data-slide-label="${esc(slide.label)}" role="group" aria-roledescription="slide" aria-label="${index+1} of ${total}: ${esc(slide.label)}"${index===0?'':' hidden'}>${img(slide.image,index===0?'fetchpriority="high" class="hero-art"':'loading="lazy" class="hero-art"')}<figcaption class="sr-only">${esc(slide.accessibleText)}</figcaption>${slide.copyOverride?`<div class="hero-copy-override"><p>${esc(slide.copyOverride.text)}</p><small>${esc(slide.copyOverride.note)}</small></div>`:''}${(slide.hotspots||[]).map(hotspot=>`<a class="hero-hotspot${hotspot.visible?' hero-hotspot-visible':''}" ${attrs(hotspot)} style="${hotspotStyle(hotspot)}" aria-label="${esc(hotspot.label)}"><span${hotspot.visible?'':' class="sr-only"'}>${esc(hotspot.label)}</span></a>`).join('')}</figure>`;
