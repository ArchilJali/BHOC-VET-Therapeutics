...
export function renderHeader(header){
  ...
  const navReturn='<a class="nav-return" '+linkAttrs(header.returnLink)+' aria-label="'+esc(header.returnLink.ariaLabel)+'"><span aria-hidden="true">←</span>'+esc(header.returnLink.menuLabel)+'</a>';
  return [
    ...
    '    </a>',
    '',
    '    <div class="header-network" aria-label="BHOC ecosystem">',
    '      <a class="site-return" '+linkAttrs(header.returnLink)+' aria-label="'+esc(header.returnLink.ariaLabel)+'">',
    '        <span class="return-arrow" aria-hidden="true">←</span>',
    '        <span>',
    '          <strong>'+esc(header.returnLink.label)+'</strong>',
    '          <small>'+esc(header.returnLink.subtitle)+'</small>',
    '        </span>',
    '      </a>',
