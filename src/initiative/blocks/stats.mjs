...
export default function renderStats(data){
  ...
  const quotation=data.quotation?'<figure class="stats-quotation"><blockquote>“<span>'+esc(data.quotation.quote)+'</span>”</blockquote><figcaption>'+esc(data.quotation.author)+', <cite>'+esc(data.quotation.work)+'</cite></figcaption></figure>':'';
  ...
  return '<section class="stats-strip" id="'+esc(data.id)+'" data-block="stats" aria-labelledby="stats-title">'+
    '<div class="shell stats-inner">'+
      '<div class="stats-copy"><p class="eyebrow" id="stats-title">'+esc(data.eyebrow)+'</p>'+quotation+'<div class="stats-grid">'+metrics+'<p class="stats-promise">'+lines(data.promiseLines)+'</p></div></div>'+
