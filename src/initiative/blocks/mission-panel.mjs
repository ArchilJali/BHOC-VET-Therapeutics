...
export default function renderMissionPanel(data){
  ...
  return '<section class="mission-panel" id="'+esc(data.id)+'" data-block="mission-panel" aria-labelledby="mission-title">'+
    '<figure class="mission-photo">'+initiativeImage(data.image)+
      '<figcaption>Photo: <a '+linkAttrs({href:data.image.source})+'>'+esc(data.image.credit)+'</a>, '+esc(data.image.license)+'</figcaption></figure>'+
