...
const logoMarkup=logo=>logo?`<span class="publication-logo-wrap"><img class="publication-logo" src="${esc(logo.url)}" alt="${esc(logo.alt||'')}" width="${Number(logo.width||80)}" height="${Number(logo.height||40)}" loading="lazy" decoding="async"></span>`:'';
